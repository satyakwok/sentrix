...
use sentrix::core::transaction::{MIN_TX_FEE, TOKEN_OP_ADDRESS, TokenOp};
...
fn test_token_deploy_and_initial_supply() {
    ...
    let contract_addr = tokens[0]["contract_address"]
        .as_str()
        .expect("contract_address field");
    assert!(
    ...
    assert_eq!(
        deployer_balance, 1_000_000,
        "deployer should receive full initial supply"
    );
}
...
fn test_contract_address_deterministic() {
    ...
    let tokens = bc.list_tokens();
    let addr1 = tokens[0]["contract_address"]
        .as_str()
        .expect("addr1")
        .to_string();
...
fn test_token_transfer() {
    ...
    let tokens = bc.list_tokens();
    let contract = tokens[0]["contract_address"]
        .as_str()
        .expect("addr")
        .to_string();
    ...
    assert_eq!(
        bc.token_balance(&contract, &deployer.address),
        deployer_before - 100
    );
    assert_eq!(
        bc.token_balance(&contract, &recipient.address),
        recv_before + 100
    );
}
...
fn test_token_burn() {
    ...
    assert_eq!(
        balance_after,
        balance_before - 50,
        "tokens should be burned from deployer balance"
    );
    ...
    assert_eq!(
        total_supply,
        1_000_000 - 50,
        "total supply must decrease after burn"
    );
}
...
fn test_mint_exceeds_max_supply_rejected() {
    ...
    assert!(
        result.is_err(),
        "block with mint exceeding max_supply must be rejected"
    );
}
...
fn test_token_transfer_exceeds_balance_rejected() {
    ...
    assert!(
        result.is_err(),
        "transfer exceeding token balance must be rejected"
    );
}
